...
if __name__ == "__main__":
    ...
    if not os.path.exists(test_pdf_path):
        ...
        try:
            ...
            from app.utils.task_utils import get_task_status,get_running_task_list
            final_state = None
            for state in kb_import_app.stream(test_state, stream_mode="values"):
                # 打印当前执行完成的节点（流式输出更直观）
                current_node = list(state.keys())[-1] if state else "未知节点"
                logger.info(f"✅ 节点执行完成：{current_node}")
                final_state = state  # 保存最终状态
                logger.info(get_running_task_list("test_kg_import_workflow_001"))
                logger.info(get_task_status("test_kg_import_workflow_001"))
